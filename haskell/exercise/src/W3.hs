...
eitherDiv x y = if y /= 0 then Right $ x `div` y else Left $ show x ++ "/0"
...
getName p = case p of Person age name -> name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge p = case p of Person age name -> age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name p = case p of Person age oldname -> Person age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = case p of Person oldage name -> Person age name
