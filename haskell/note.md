...
Haskell中所有的函数都可以看作是单参数的函数。函数进行部分应用时，返回一个新的函数表示剩运算。  
...
::的优先级在各种符号中最低，但\ ... -> ... :: Type中的Type指的不是整个匿名函数的类型，而是->右侧表达式的类型，其实这是因为即Type被函数体包含进去了， 所以如果需要标注整个匿名函数的类型， 请加上括号，如：  
```haskell
(\x -> x + 1) :: Int-> Int  
```  
```haskell  
...
bool是stack里两个地址，只有Info table指向底层数据    

## 模式匹配
对拥有多于一个参数的构造函数的模式进行匹配时，括号是必须的。  
...
## newtype
newtype和有单参数单一构造函数的data都定义一个单值的构造函数。但是newtype定义的构造函数是严格的，而后者则是惰性的。  
...
```
## 类型类   
结构相等是各个域对应相等，deriving (Eq)是结构相等  
...
instance Eq A where
    ...
    x == y = ...
```  
Specialise是特别的拷贝，提前生成好，不再每次遇到具体类型时生成，不同类型的机器码不同。   
...
# Monad
Applicative的局限，一旦计算被升格，之后就无法更改被函子f包裹的计算内容了。  
`<*>`要求被包裹之后的函数是f (a -> b)类型的，即升格之后的函数不应该添加额外的上下文信息，只能携带外层的函子信息。  
在使用应用函子构造计算时，我们无法在计算中途更改应用函子包含的上下文信息，这些信息由参数静态决定。这并非完全是坏事，静态决定函子上下文往往能带来一些优化的机会。  
```Haskell
class Applicative m => Monad (m :: * -> *) where  -- * -> * type到type
(>>=) :: m a -> (a -> m b) -> m b 
x >>= f = join $ fmap f x
join :: m (m a) -> m a
join mma = mma >>= id  
(>=>) :: Monad m => (a -> m b) -> (b -> m c) -> a -> m c 	-- aka Kleisli composition
```  
bind函数的名字是因为它可以方便地粘合多个产生函子上下文的计算。  
return函数只是用来把一个值升格到函子范畴以满足计算的要求，同时添加的函子包裹不会影响上下文的计算语义。  
Monad要求return是>>=的左、右单位元，并且>>=满足结合性，即 m >>= (\x -> k x >>= h) = (m >>= k) >>= h  
Monad有一些代码等价转化的感觉，可能有一些优化。  
 m和h是两个Monad
Haskell中，要求对于同一个类型，如果它既是应用函子的实例，也是单子的实例，那么不管是使用应用函子还是单子来构建计算，得到的计算语义应该完全相同。只是有一些计算只能使用单子来构建，而有些运算使用应用函子会提供额外优化的机会。  
map f m = m >>= (return . f)
文献中 >>=叫做bind，return叫做unit。  
把... >>= \param ->的过程写作param <- ...，表示从函子包裹中取出值绑定给param。  
### do语法  
...
do语法的计算语义取决于具体的函子类型。真正的关键在单子定义的>>=上。  
do语法中没有<-的语句代表不用取出包裹里的值，后面的运算会直接把新的值填充进包裹。  
do语法中，任何一个包裹在函子中的表达式都代表忽略之前容器里的内容，而是让表达式的值来填充新的函子。  
do语法中需要临时绑定时，可以使用let。do中的let...其实就是let... in...语法，do段落里面的let产生的绑定在它后面的表达式中有效。  
```Haskell
(*>) :: Applicative f => f a -> f b -> f b			-- 保留左侧函子包裹的信息和右侧的值
(<*) :: Applicative f => f a -> f b -> f a		   -- 保留右侧函子包裹的信息和左侧的值
(>>) :: Monad m => m a -> m b -> m b
mx >> my = mx >>= (\x -> my >>= \y -> return y)
```  
IO作为单子的实例，其含义在于保证IO携带的副作用在外界环境中执行。  
IO中，>>=的作用是保证“副作用”的顺序执行。表达式的求值顺序并不是由书写顺序决定的，一个绑定出现的位置并不会影响最终的计算结果。>>=保证了函子中包含的“外部世界”这个上下文被顺序从左侧传递到右侧。  
```Haskell
main :: IO ()
main = do
 x <- readLn
 y <- readLn
 print $ x && y		-- x是False时，让然需要继续读取y，IO函子计算语义决定的
```  
Control.Monad模块中，定义了一对函数来帮助书写带有条件分支的单子操作。  
```Haskell
...
void x = () <$ x				-- 可以在任意单子运算里代替return ()，用于舍弃x中结果
```
使用盒子是因为经常在计算时打标签，来获得额外的信息。  
Applicative有上下文组合的语义。  
每一个instance有自己组合上下文的方式。  

join :: m (m a ) -> m a  
jojn fmap <$> f a  

concat $ (\x -> [4, 5, 6] >>= <$> [1, 2, 3])  
concat $ [ (\x -> ...) 1, (\x -> ...) 2, (\x -> ...) 3]  
<-允许抽象地思考函子的问题  
x <- m a中，x一定是a类型，提取了函子中的值，传递了函子的上下文。  
Monad最后的上下文是原来两个上下文的组合  
m a -> (a -> m b) -> m b  
flatmap -> xs.map(neighbors).flatten  
concatMap :: Foldable t => (a -> [b]) -> t a -> [b]   
`->`和`<-`左侧都是模式匹配
### 控制结构
```Haskell
class (Alternative m, Monad m) => MonadPlus m where				-- 选择单子类型类
 mzero :: m a
 mplus :: m a -> m a -> m a  
 
 sequence :: (Traversable t, Monad m) => t (m a) -> m (t a )		-- 用于顺序连接任意长度的单子运算
 
 sequence_ :: (Foldable t, Monad m) => t (m a) -> m ()			  -- 用于顺序连接任意长度单子运算，但不保留结果

mapM :: (Traversable t, Monad m) => (a -> m b) -> t a -> m (t b)	-- 代替sequence.map的组合

mapM_ :: (Foldable t, Monad m) => (a -> m b) -> t a -> m ()			-- 代替sequence_.map的组合

forM :: (Traversable t, Monad m) => t a -> (a -> m b) -> m (t b)	-- 用flip交换了mapM的参数位置
 
forM_ :: (Foldable t, Monad m) => t a -> (a -> m b) -> m ()	-- 用flip交换了mapM_的参数位置
...
foldM :: (Foladable t, Monad m) => (b -> a -> m b) -> b -> t a -> m b				-- 合理使用可解决递归问题，从左到右
foldM_ :: (Foldable t, Monad m) => (b -> a -> m b) -> b -> t a -> m ()
```
依赖还有realworld token，main:: IO ()展开为main RW -> (RW, ())，运行时会给提供第一个RW  
RW这个token不会占用栈空间，是反复利用之前的  
:set -XDeriveFunctor

foldM (\acc x -> print acc >> return (acc ++ x)) 
Haskell允许使用子类方法实现父类的方法，比如如果是Monad，那么Applicative的<\*>可定义为ap。
