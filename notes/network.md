TCP的数据需要经过协议切割  
FTP两个连接，控制连接、数据连接。如何处理二者的对应关系？  
状态编号：如果没有，没办法判断服务器出错了还是结果就是这样？  
echo, chargen  
# 
...
- getnameinfo(const struct sockaddr \*sa, sockaddr \*sa, socklen_t salen, char \*host, size_t hostlen, char \*serv, size_t servlen, int flags)  
与`getaddrinfo`相反，由地址得到主机和服务名，分别在host和serv中，hostlen和servlen是二者分别的最大长度。flags常用的有NI_NOFQDN（host只包含主机名，不包含域名），NI_NAMEREQD（如果DNS找不到主机名，那么函数调用失败，默认找不到会将IP地址的字符串形式存入host返回）
...
发送出去但还没被确认的数据称为outstanding.  
EINTR：许多系统调用执行时如果被信号打断，那么返回EINTR，因为系统调用无法继续进行。通常的处理方法是重试一次系统调用，因为实际并没有错误发生。
# 问题
...
* signal mask  
* 优先级  
2000年左右中国大部分电脑用的都是私有地址，在防火墙之后，两台主机通信，需要经过服务器。服务器在美国，这样速度非常慢。后期丢包比较严重，因为NAT端口映射可能会变，服务器看到端口号改变后，会重置连接。  
使用UDP，服务器只在开始时用于通信双方登记公网端口号，无需经过服务器中转。  
STUN（Session Traversal Utilities for NAT，NAT会话穿越应用程序）：通过公网的STUN服务器帮助，让私网内主机获得自己在公网的IP和端口号。  
symmetric NAT：私网同一个主机发的包到不同目的时映射到不同NAT的端口，这样通信会有问题。  
Anycast：同一个IP目的地址，多条路径，多个目的端，网络根据代价等从中选择一个。  
UDP中recvfrom可以获得对端地址，可以用于安全性验证以及数据源的区分（多个客户同时存在时）。
UDP不需要为每一个客户开一个线程？？？每一个用户只需做步进  
迅雷的突破是用UDP把文件传输重写了一遍，这样即使在私网里也可以成为种子，符合中国的情况。  
UDP服务器端可以无状态，来请求就处理，丢失交给客户端处理。这样服务器比较简单，并且负载能力也会提高。  
race condition：程序的实际执行次序与预想的不同。  
recvfrom可能接受到payload为0的数据包。  
SO\_RCVTIMEO后期使用不多，因为如果是多个线程的话，那么所有的线程选项都必须相同。  
select不同线程的超时限制可以不同。  
DNS序列号太短，所以容易被攻击。  
清华有UDP chargen服务。  
抢票，抢红包大部分都在客户端过滤了。  