...
def create_dataset(dataset, augment, fold=1, reset=False, suffix=''):
    # augment=1, fold=(1,2,3,4), reset=0, suffix=''
    root = 'data/%s/' % dataset
    num_workers = 5
    image_size = 1720
    # a~z, 0~9
    ...
    phoc_levels = list(range(1, 6))
    # washington_foldX
    dataset_full = dataset + '_fold%d' % fold
    # data/washington/washington_foldX/
    ...
    if augment:
        ...
        full_page_data = full_page_augment(data, augmentation_directory, tparams, num_images=num_images / 2,
                                           reset=reset)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset', help='The dataset used for training, one of "washington", "iam", '
                             '"washington_small", or "iiit_hws"')
