...
class CASIA(Dataset):
    ...
    def __init__(self, image_dir, label_dir, label_file, char_class):
    ...
    @staticmethod
    def get_image(img_path, img_name):
        ...
        try:
            ...
            img = cv2.resize(img, dsize=(216, 216), interpolation=cv2.INTER_CUBIC)
        else:
            img = cv2.resize(img[min_x: max_x, min_y: max_y], dsize=(216, 216), interpolation=cv2.INTER_CUBIC)
        # put the resized character into the center of the image background with target size
        h, w = img.shape
        img_bg = np.full((224, 224), 255, dtype='uint8')
