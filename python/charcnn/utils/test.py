...
def test(method, query_word, file_name, progressbar, image_path=None):
    ...
    if method == 'qbs':
        query_img = generate_character(query_word, size['googlenet']['image_size'], size['googlenet']['image_size'], 0,
                                       './utils/fonts/fangzheng_heiti.TTF', 0)
    else:
        query_img = get_image(cv2.imread(image_path, 0), size['googlenet']['char_size'],
                              size['googlenet']['image_size'])
    query_img = transforms.ToTensor()(query_img).unsqueeze_(0)
    query_img.to(device)
    # get the document image names and the corresponding label names
    segmented = segment(file_name)
    print(method, query_word, file_name)
    doc, image_lines, row_starts, total = segmented['image'], segmented['image_lines'], segmented['row_start'], segmented['total']
    print(total)
    ...
    for index, image_line in enumerate(image_lines):
        ...
        for column_start in range(w - h + 1):
            doc_line = get_image(image_line[:, column_start: column_start + h], size['googlenet']['char_size'],
                                 size['googlenet']['image_size'])
            with torch.no_grad():
    ...
    filename = os.path.join(path, name + 'result' + '.' + suffix)
    Image.fromarray(doc).save(filename)
    return filename
...
def parse_arg():
    ...
    parser.add_argument("--font_path", "-font", default='./utils/fonts/fangzheng_heiti.TTF',
                        help="Path to the font used to generate" +
